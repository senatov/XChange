...
public class KucoinMarketDataServiceIntegration {
	...
	public void testGetTicker() throws Exception {
		...
		assertThat(ticker.getVolume()).isNotNull().isGreaterThanOrEqualTo(ZERO);
		assertThat(ticker.getQuoteVolume()).isNotNull().isGreaterThanOrEqualTo(ZERO);
		assertThat(ticker.getInstrument()).isEqualTo(ETH);
	...
	public void testGetTickers() throws Exception {
		...
		assertThat(tickerResponse.getVol()).isNotNull().isGreaterThanOrEqualTo(ZERO);
		assertThat(tickerResponse.getVolValue()).isNotNull().isGreaterThanOrEqualTo(ZERO);
	}
	...
	private void checkOrderBookIntegrity(OrderBook orderBook) {
		...
		for (LimitOrder o : orderBook.getBids()) {
			...
			assertNotEquals(0, o.getOriginalAmount().compareTo(ZERO));
		}
		previousPrice =
	...
	public void testKlines() throws Exception {
		...
		assertThat(first.getOpen()).isEqualTo(valueOf(10411.5));
		assertThat(first.getHigh()).isEqualTo(valueOf(10411.5));
		assertThat(first.getLow()).isEqualTo(valueOf(10396.3));
		assertThat(first.getClose()).isEqualTo(valueOf(10401.9));
		assertThat(first.getVolume()).isEqualTo(valueOf(29.11357276));
		assertThat(first.getAmount()).isEqualTo(valueOf(302889.301529914));
		...
		assertThat(last.getOpen()).isEqualTo(valueOf(10089));
		assertThat(last.getHigh()).isEqualTo(valueOf(10092.1));
		assertThat(last.getLow()).isEqualTo(valueOf(10087.7));
		assertThat(last.getClose()).isEqualTo(valueOf(10088.7));
		assertThat(last.getVolume()).isEqualTo(valueOf(5.12048315));
		assertThat(last.getAmount()).isEqualTo(valueOf(51658.509394017));
	}
}
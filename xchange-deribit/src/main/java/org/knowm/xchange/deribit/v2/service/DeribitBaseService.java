	...
	private DeribitAuthentication authOverClientSignature(String clientId, String clientSecret)
		...
		try {
			...
			final SecretKey secretKey =
					new SecretKeySpec(clientSecret.getBytes(StandardCharsets.UTF_8), BaseParamsDigest.HMAC_SHA_256);
			mac.init(secretKey);
		...
		String signature = DigestUtils.bytesToHex(mac.doFinal(toSign.getBytes(StandardCharsets.UTF_8))).toLowerCase();
		return deribit
				.auth(

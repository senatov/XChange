	...
	public void checkBalance(Order order, BigDecimal bidAmount) {
		switch (order.getType()) {
			case ASK:
				...
				Balance askBalance =
						balances.computeIfAbsent(order.getInstrument().getBase(), this::defaultBalance).get();
				checkBalance(order, askAmount, askBalance);
				break;
			case BID:
				Balance bidBalance =
						balances.computeIfAbsent(order.getInstrument().getCounter(), this::defaultBalance).get();
	...
	private void reserve(LimitOrder order, boolean negate) {
		switch (order.getType()) {
			case ASK:
				...
				balance(order.getInstrument().getBase())
						.updateAndGet(
								b -> {
			...
			case BID:
				...
				balance(order.getInstrument().getCounter())
						.updateAndGet(
								b -> {
									if (b.getAvailable().compareTo(bidAmount) < 0) {
										throw new ExchangeException(
												"Insufficient balance: "
														...
														+ order.getInstrument().getCounter()
														+ " required but only "
														+ b.getAvailable()
														+ " available");
									}

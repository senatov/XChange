...
import static org.knowm.xchange.currency.CurrencyPair.BTC_EUR;
...
import static org.knowm.xchange.dto.Order.OrderStatus.PENDING_NEW;
import static org.knowm.xchange.dto.Order.OrderStatus.STOPPED;
...
public class CoinbaseProAdaptersTest {
	...
	public void testTickerAdapter() throws IOException {
		...
		Ticker ticker =
				CoinbaseProAdapters.adaptTicker(coinbaseExTicker, coinbaseExStats, BTC_USD);
	...
	public void testTradeHistoryAdapter() throws IOException {
		...
		assertThat(trade.getInstrument()).isEqualTo(ETH_BTC);
	...
	public void testOrderStatusMarketOrderFilled() throws IOException {
		...
		assertThat(order.getInstrument()).isEqualTo((BTC_USD));
	...
	public void testOrderStatusStopOrderNew()
		...
		assertThat(order.getInstrument()).isEqualTo((BTC_EUR));
	...
	private void assertLimitOrderPending(final Order order) {
		assertThat(order.getStatus()).isEqualTo(PENDING_NEW);
		assertThat(order.getId()).isEqualTo("b2cdd7fe-1f4a-495e-8b96-7a4be368f43c");
		assertThat(order.getInstrument()).isEqualTo((BTC_USD));
		assertThat(order.getOriginalAmount()).isEqualByComparingTo(new BigDecimal("0.07060351"));
		assertThat(order.getCumulativeAmount()).isEqualByComparingTo(BigDecimal.ZERO);
	...
	public void testAdaptProductID() {
		String productID = CoinbaseProAdapters.adaptProductID(ETH_BTC);

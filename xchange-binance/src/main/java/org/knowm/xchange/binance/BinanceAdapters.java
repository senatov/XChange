...
import static org.knowm.xchange.binance.dto.trade.OrderType.LIMIT_MAKER;
import static org.knowm.xchange.binance.dto.trade.OrderType.MARKET;
import static org.knowm.xchange.binance.dto.trade.OrderType.STOP;
import static org.knowm.xchange.binance.dto.trade.OrderType.STOP_LOSS;
import static org.knowm.xchange.binance.dto.trade.OrderType.STOP_LOSS_LIMIT;
import static org.knowm.xchange.binance.dto.trade.OrderType.STOP_MARKET;
import static org.knowm.xchange.binance.dto.trade.OrderType.TAKE_PROFIT;
import static org.knowm.xchange.binance.dto.trade.OrderType.TAKE_PROFIT_LIMIT;
import static org.knowm.xchange.binance.dto.trade.OrderType.TAKE_PROFIT_MARKET;
import static org.knowm.xchange.currency.Currency.BNB;
import static org.knowm.xchange.currency.Currency.BTC;
import static org.knowm.xchange.currency.Currency.ETH;
import static org.knowm.xchange.currency.Currency.IOT;
...
public class BinanceAdapters {
	...
	public static CurrencyPair convert(String symbol) {
		...
		for (Currency base : Arrays.asList(BTC, ETH, BNB, USDT)) {
		...
		switch (order.getIntention()) {
			case STOP_LOSS:
				return order.getLimitPrice() == null
						? (order.getInstrument() instanceof CurrencyPair) ? STOP_LOSS : STOP_MARKET
						: (order.getInstrument() instanceof CurrencyPair) ? STOP_LOSS_LIMIT : STOP;
			case TAKE_PROFIT:
				return order.getLimitPrice() == null
						? (order.getInstrument() instanceof CurrencyPair) ? TAKE_PROFIT : TAKE_PROFIT_MARKET
						: (order.getInstrument() instanceof CurrencyPair) ? TAKE_PROFIT_LIMIT : TAKE_PROFIT;
			default:
						...
						.map(
								t ->
										new UserTrade.Builder()
												...
												.feeCurrency(getInstance(t.commissionAsset))
												.build())
						.collect(Collectors.toList());
	...
	public static FundingRate adaptFundingRate(BinanceFundingRate binanceFundingRate) {
		return new FundingRate.Builder()
				.fundingRate1h(binanceFundingRate.getLastFundingRate().divide(BigDecimal.valueOf(8), binanceFundingRate.getLastFundingRate().scale(), HALF_EVEN))
				...
				.fundingRateEffectiveInMinutes(MILLISECONDS.toMinutes(binanceFundingRate.getNextFundingTime().getTime() - binanceFundingRate.getTime().getTime()))
				.build();
	}
}
	...
	static class CoinMarketCapTickerDeserializer extends JsonDeserializer<CoinMarketCapTicker> {
		...
		public CoinMarketCapTicker deserialize(JsonParser jp, DeserializationContext ctxt)
			...
			if (node.isObject()) {
				...
				BigDecimal circulatingSupply = BigDecimal.valueOf(node.get("circulating_supply").asDouble());
				BigDecimal totalSupply = BigDecimal.valueOf(node.get("total_supply").asDouble());
				BigDecimal maxSupply = BigDecimal.valueOf(node.get("max_supply").asDouble());

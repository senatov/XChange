	...
	static class CoinMarketCapQuoteDeserializer extends JsonDeserializer<CoinMarketCapQuote> {
		...
		public CoinMarketCapQuote deserialize(JsonParser jp, DeserializationContext ctxt)
			...
			if (node.isObject()) {
				...
				BigDecimal volume24h = BigDecimal.valueOf(node.get("volume_24h").asDouble());
				BigDecimal marketCap = BigDecimal.valueOf(node.get("market_cap").asDouble());
				// TODO use these to create CoinMarketCapHistoricalSpotPrice instances
				BigDecimal pctChange1h = BigDecimal.valueOf(node.get("percent_change_1h").asDouble());
				BigDecimal pctChange24h = BigDecimal.valueOf(node.get("percent_change_24h").asDouble());
				BigDecimal pctChange7d = BigDecimal.valueOf(node.get("percent_change_7d").asDouble());

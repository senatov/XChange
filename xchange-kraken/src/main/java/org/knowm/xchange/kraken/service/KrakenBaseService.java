...
public class KrakenBaseService extends BaseExchangeService implements BaseService {
	...
	protected final KrakenAuthenticated kraken;
	...
	public KrakenBaseService(Exchange exchange) {
		super(exchange);
		kraken = ExchangeRestProxyBuilder.forInterface(KrakenAuthenticated.class, exchange.getExchangeSpecification()).build();
	 ...
	 * <p><a href="https://support.kraken.com/hc/en-us/articles/360001491786-API-Error-Codes">...</a>
	 */
	public <R> R checkResult(KrakenResult<R> krakenResult) {
		if (!krakenResult.isSuccess()) {
			...
			switch (error) {
				case "EAPI:Invalid nonce" -> throw new NonceException(error);
				case "EGeneral:Temporary lockout" -> throw new FrequencyLimitExceededException(error);
				case "EOrder:Insufficient funds" -> throw new FundsExceededException(error);
				case "EGeneral:Too many requests", "EAPI:Rate limit exceeded", "EOrder:Rate limit exceeded" ->
						throw new RateLimitExceededException(error);
				case "EService:Unavailable", "EService:Busy" -> throw new ExchangeUnavailableException(error);
			}
	...
	protected String delimitAssets(Currency[] assets) {
			...
			for (Object item : items) {
				if (item instanceof KrakenOrderFlags) {
					if (delimitStringBuilder == null) {
						...
						delimitStringBuilder.append(",").append(item);
					}
				}
			}
			if (delimitStringBuilder != null) {

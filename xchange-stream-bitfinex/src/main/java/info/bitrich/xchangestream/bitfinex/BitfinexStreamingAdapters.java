...
class BitfinexStreamingAdapters {
	...
	static BitfinexWebSocketAuthPreTrade adaptPreTrade(JsonNode preTrade) {
		if (preTrade.size() < 12) {
			LOG.error(
					"addPreTrade unexpected record size={}, record={}", preTrade.size(), preTrade);
			return null;
		}
	...
	static BitfinexWebSocketAuthTrade adaptTrade(JsonNode trade) {
		if (trade.size() < 11) {
			LOG.error("addTrade unexpected record size={}, record={}", trade.size(), trade);
			return null;
		}
	...
	private static BitfinexWebSocketAuthOrder createOrderObject(JsonNode order) {
		if (order.size() < 32) {
			LOG.error(
					"createOrderObject unexpected record size={}, record={}", order.size(), order);
			return null;
		}
